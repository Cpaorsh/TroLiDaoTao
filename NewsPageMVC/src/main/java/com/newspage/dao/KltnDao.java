...
public class KltnDao {
	...
	public int saveKltn(Kltn s){    
	    String sql="insert into kltn(detai, mgv, msv) values('"+s.getDetai()+"', '"+s.getMgv()+"',  '"+s.getMsv()+"')";    
	    return template.update(sql);    
	}    
	public int updateKltn(Kltn s){    
	    String sql="update kltn set detai='"+s.getDetai()+"', mgv='"+s.getMgv()+"', msv='"+s.getMsv()+"', duyet='"+s.getDuyet()+"' where mkl="+s.getMkl()+"";    
	    return template.update(sql);    
	}    
	...
	public List<Kltn> getKltns(int id){    
	    return template.query("select * from kltn INNER JOIN giangvien ON kltn.mgv = giangvien.mgv INNER JOIN sinhvien ON kltn.msv = sinhvien.msv where idhk="+id+"",new RowMapper<Kltn>(){    
